...
impl NativeNote {
    pub fn new(mut cx: FunctionContext) -> JsResult<JsBox<NativeNote>> {
        ...
        let value_u64: u64 = value.parse::<u64>().or_else(|err| cx.throw_error(err.to_string()))?;
    ...
    pub fn deserialize(mut cx: FunctionContext) -> JsResult<JsBox<NativeNote>> {
        ...
        let note = cx.borrow(&bytes, |data| {
            Note::read(data.as_slice(), hasher)
        }).or_else(|err| cx.throw_error(err.to_string()))?;
    ...
    pub fn serialize(mut cx: FunctionContext) -> JsResult<JsBuffer> {
        let note = cx.this().downcast_or_throw::<JsBox<NativeNote>, _>(&mut cx)?;
    ...
    pub fn nullifier(mut cx: FunctionContext) -> JsResult<JsBuffer> {
        ...
        let position_u64 = position.parse::<u64>().or_else(|err| cx.throw_error(err.to_string()))?;

        let private_key =
            Key::from_hex(SAPLING.clone(), &owner_private_key).or_else(|err| cx.throw_error(err.to_string()))?;
