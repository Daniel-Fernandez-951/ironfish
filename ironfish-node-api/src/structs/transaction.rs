...
impl NativeTransactionPosted {
    pub fn deserialize(mut cx: FunctionContext) -> JsResult<JsBox<NativeTransactionPosted>> {
        ...
        let transaction = cx.borrow(&bytes, |data| {
    ...
    pub fn serialize(mut cx: FunctionContext) -> JsResult<JsBuffer> {
        let transaction = cx.this().downcast_or_throw::<JsBox<NativeTransactionPosted>, _>(&mut cx)?;

        let mut arr: Vec<u8> = vec![];
        transaction.transaction.write(&mut arr).or_else(|err| cx.throw_error(err.to_string()))?;
    ...
    pub fn transaction_fee(mut cx: FunctionContext) -> JsResult<JsString> {
        let transaction = cx.this().downcast_or_throw::<JsBox<NativeTransactionPosted>, _>(&mut cx)?;
    ...
    pub fn transaction_signature(mut cx: FunctionContext) -> JsResult<JsBuffer> {
        ...
        cx.borrow_mut(&mut bytes, |data| {
            let slice = data.as_mut_slice();
            for i in 0..slice.len() {
                slice[i] = serialized_signature[i];
            }
        });
    ...
    pub fn transaction_hash(mut cx: FunctionContext) -> JsResult<JsBuffer> {
        let transaction = cx.this().downcast_or_throw::<JsBox<NativeTransactionPosted>, _>(&mut cx)?;
...
impl NativeTransaction {
    ...
    pub fn receive(mut cx: FunctionContext) -> JsResult<JsString> {
        let transaction = cx.this().downcast_or_throw::<BoxedNativeTransaction, _>(&mut cx)?;
        let spender_hex_key = cx.argument::<JsString>(0)?.value(&mut cx);
        let note = cx.argument::<JsBox<NativeNote>>(1)?;

        let spender_key =
            Key::from_hex(SAPLING.clone(), &spender_hex_key).or_else(|err| cx.throw_error(err.to_string()))?;
        transaction.borrow_mut().transaction
    ...
    pub fn spend(mut cx: FunctionContext) -> JsResult<JsString> {
        ...
        let spender_key =
            Key::from_hex(SAPLING.clone(), &spender_hex_key).or_else(|err| w.cx.borrow_mut().throw_error(err.to_string()))?;
        transaction.borrow_mut().transaction
    ...
    pub fn post(mut cx: FunctionContext) -> JsResult<JsBox<NativeTransactionPosted>> {
        ...
        let intended_transaction_fee_u64 = intended_transaction_fee.parse::<u64>().or_else(|err| cx.throw_error(err.to_string()))?;

        let spender_key =
            Key::from_hex(SAPLING.clone(), &spender_hex_key).or_else(|err| cx.throw_error(err.to_string()))?;
        let change_key = if change_goes_to != "" {
            Some(PublicAddress::from_hex(SAPLING.clone(), &change_goes_to).or_else(|err| cx.throw_error(err.to_string()))?)
        } else {
            None
        };
...
impl NativeSimpleTransaction {
    ...
    pub fn spend(mut cx: FunctionContext) -> JsResult<JsString> {
        let transaction = cx.this().downcast_or_throw::<BoxedNativeSimpleTransaction, _>(&mut cx)?;
    ...
    pub fn post(mut cx: FunctionContext) -> JsResult<JsBox<NativeTransactionPosted>> {
        ...
        let posted_transaction = transaction.borrow_mut().transaction.post().or_else(|err| cx.throw_error(err.to_string()))?;
        Ok(cx.boxed(NativeTransactionPosted {
            transaction: posted_transaction,
        }))
    }
